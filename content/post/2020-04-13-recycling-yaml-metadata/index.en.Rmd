...
The `metadata` object returns the YAML metadata of the __current__ R Markdown document as a list. Keep in mind that this object is only created in the knit environment, i.e. when you render your document. For example, this post has the following metadata specified at the top of the `Rmd` file:

```{r, echo = FALSE, comment = "", class.output = "yaml"}
...
```
> This post was written on `r knitr::inline_expr("format(as.Date(metadata$date), '%B %d, %Y')")`.
```
...
```{r}
yaml_front_matter("index.en.Rmd") # provide file path
```
